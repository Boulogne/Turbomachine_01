...
Wt_LPT=Wf/(eff_m);
To6=To5-Wt_LPT/(mdot*Cpa);
Po6=Po5*(To5/To6)^(-(gamma_a)/(eff_pt*(gamma_a-1)));
...
T7=To6/(1+gamma_a*R_g/2*Cpa);
V7=M7*sqrt(gamma_a*R_g*T7);
T7S=To6*(1-1/eff_nc)+T7/eff_nc;
P7cr=Po6*(T7S/To6)^(gamma_a/(gamma_a-1)); 
P7=P7cr;
...
if P8cr<Pa   %Pa % Then the flow is unchocked  
    P8=Pa;
    T8S=To2*(P8/Po2)^(gamma_g/(gamma_g-1));
    T8=To2-(To2-T8S)*eff_nb;
    V8=sqrt((To2-T8)*2*Cpg);
